...
def process_single_row(row,tokenizer, add_answer=False, start_answer_txt=None):
    try:
...
class AyaChatPromptTemplate:
    ...
    def create_token_labels(
        ...
        inps_full_txt = self.apply_chat_prompt(messages)
        inps_full_ids = self.tokenizer.encode(inps_full_txt, add_special_tokens=False)

        inps_prefix_txt = self.apply_chat_prompt(messages[:-1])
