...
class Qwen3_5DPOConfig(DPOConfig):
    def __init__(self, **kwargs):
        accepted_keys = self._accepted_config_keys()
        dpo_kwargs = {key: value for key, value in kwargs.items() if key in accepted_keys}
        extra_kwargs = {key: value for key, value in kwargs.items() if key not in accepted_keys}

        super().__init__(**dpo_kwargs)

        # Keep newer TRL config values available even if the installed version
        # does not accept them in DPOConfig.__init__ yet.
        for key, value in extra_kwargs.items():
            setattr(self, key, value)

    @staticmethod
    def _accepted_config_keys() -> set[str]:
        if not is_dataclass(DPOConfig):
            return set()
        return {field.name for field in fields(DPOConfig) if field.init}
